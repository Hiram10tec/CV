...
import { BrainCircuit, Boxes, ExternalLink, FolderKanban, FlaskConical, Smartphone } from "lucide-react";
          ...
          {projects.items.map((project, index) => {
            ...
            return (
              <Reveal key={project.title} delay={index * 0.08}>
                ...
                <article
                  className="group tron-card tron-card-scan relative h-full overflow-hidden transition-[border-color,box-shadow] duration-300 hover:border-violet-500/40 rounded-[4px]"
                >
                  ...
                  <div className="relative aspect-[16/10] overflow-hidden border-b border-cyan-400/15">
                    ...
                    <div
                      className="absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center border border-cyan-400/25 bg-abyss/70 text-cyan-300 backdrop-blur rounded-[2px]"
                    >
                  ...
                  <div className="p-6">
                    ...
                    {project.role && (
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        ...
                        <span className="font-tron text-[0.6rem] uppercase tracking-[0.18em] text-steel">{project.company}</span>
                        <span className="text-cyan-400/30">·</span>
                        <span className="font-tron text-[0.6rem] uppercase tracking-[0.18em] text-steel/60">{project.period}</span>
                      </div>
                    )}
                    <p className="mt-3 text-sm leading-7 text-steel">{project.description}</p>

                    <div className="mt-5 border border-cyan-400/12 bg-cyan-400/3 p-4 rounded-[2px]">
                      <p className="font-tron text-[0.58rem] font-semibold uppercase tracking-[0.22em] text-cyan-400/70">
                        {projects.highlightLabel}
                      </p>
                      <p className="mt-2 text-sm leading-7 text-steel">{project.highlight}</p>
                    </div>

                    <div className="mt-3 border border-cyan-400/12 bg-cyan-400/3 p-4 rounded-[2px]">
                      <p className="font-tron text-[0.58rem] font-semibold uppercase tracking-[0.22em] text-cyan-400/70">
                        {projects.resultLabel}
                      </p>
                      <p className="mt-2 text-sm leading-7 text-frost">{project.result}</p>
                    </div>
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      {project.tags.map((tag) => (
                        <span
                          key={tag}
                          className="font-tron border border-cyan-400/18 bg-cyan-400/4 px-3 py-1 text-[0.6rem] uppercase tracking-[0.12em] text-steel rounded-[2px]"
                        >
                          {tag}
                        </span>
                      ))}
                      ...
                      <div className="mt-4 border-t border-cyan-400/10 pt-4 flex flex-wrap gap-2">
                        ...
                        {project.links.map((link) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="font-tron inline-flex items-center gap-1.5 border border-cyan-400/25 bg-cyan-400/5 px-3 py-1 text-[0.6rem] uppercase tracking-[0.12em] text-cyan-300 transition hover:border-cyan-400/50 hover:bg-cyan-400/10 rounded-[2px]"
                          >
